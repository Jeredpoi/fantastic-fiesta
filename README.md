...
YouTube не доверяет IP-адресам хостингов (Replit, VPS и т.п.) и на публичные
видео отвечает «Sign in to confirm you're not a bot». Лечится cookies от
залогиненного аккаунта:

1. Заведите **отдельный** Google-аккаунт (свой основной лучше не использовать —
   YouTube может ограничить аккаунт за автоматизацию).
2. Войдите в него в браузере, откройте youtube.com.
3. Расширением «Get cookies.txt LOCALLY» (Chrome/Firefox) экспортируйте cookies
   в формате Netscape.
4. Загрузите файл на сервер (на Replit — перетащите в файлы проекта) и укажите
   в `.env`: `COOKIES_FILE=./cookies.txt`.
5. Перезапустите бота. Файл не коммитьте в git (`.gitignore` уже прикрывает
   `cookies.txt`).

Cookies живут недели-месяцы; если ошибка вернулась — переэкспортируйте файл.
